...
$hostname = "localhost";
$username = "techulus_fmb";
$dbname = "techulus_fmb";

$con = mysql_connect($hostname,$username,"1ament@lisT");
if (!$con)
  {
...
$data_p = mysql_query("SELECT * FROM `listing` WHERE `id`='".$id."'") or die(mysql_error()); 
 while($info = mysql_fetch_array( $data_p )) 

 { 
if($info['photo1']) {
	$file="uploads/".$info['photo1'];
	unlink($file);
	$file="uploads/thumb_".$info['photo1'];
	unlink($file);
	 } 
}
...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	if($image_res){
		...
		$new_file_name = 'findmyboat'. '_' .  rand(0, 9999999999) . '_' .  rand(0, 9999999999) . '.' . $image_extension;
		...
		if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality))
		{
			...
			mysql_query("UPDATE `listing` SET `photo1`='$new_file_name' WHERE `id`='".$id."'") or die(mysql_error());
...
function crop_image_square($source, $destination, $image_type, $square_size, $image_width, $image_height, $quality){
	...
	if(imagecopyresampled($new_canvas, $source, 0, 0, $x_offset, $y_offset, $square_size, $square_size, $s_size, $s_size)){
		save_thumb($new_canvas, $destination, $image_type, $quality);
	}
...
function save_image($source, $destination, $image_type, $quality){
	$stamp = imagecreatefrompng('fmb.png');
	$marge_right = 10;
	$marge_bottom = 10;
	$sx = imagesx($stamp);
	$sy = imagesy($stamp);
...
##### Saves image resource to file ##### 
function save_thumb($source, $destination, $image_type, $quality){
	switch(strtolower($image_type)){//determine mime type
		case 'image/png': 
			imagepng($source, $destination); return true; //save png file
			break;
